...
#define COLUMNS "endpoint_id, mobilus_device_id, unique_id, name, state"
...
void SqliteSwitchRepository::save(const Switch& switch_)
{
    ...
    stmt->bind(3, switch_.uniqueId().value());
    stmt->bind(4, switch_.name());
    stmt->bind(5, static_cast<uint8_t>(switch_.state()));
...
Switch SqliteSwitchRepository::mapRowTo(sqlite::Statement& stmt)
{
    ...
    return Switch::restoreFrom(
        ...
        static_cast<Switch::State>(stmt.columnAsUint8(4))
    );
    // clang-format on
}
